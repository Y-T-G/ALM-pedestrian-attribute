...
# https://github.com/pytorch/pytorch/issues/30563#issuecomment-1295761759
def affine_grid(theta, size, device='cuda', align_corners=False):
    N, C, H, W = size
...
class ChannelAttn(nn.Module):
    def __init__(self, in_channels, pooling_size, reduction_rate=16):
    ...
    def forward(self, x):
        ...
        x_size = [self.pooling_size, self.pooling_size//2]
        x = F.avg_pool2d(x, x_size)
...
class SpatialTransformBlock(nn.Module):
    def __init__(self, num_classes, pooling_size, channels):
        ...
        for i in range(self.num_classes):
            ...
            self.att_list.append(ChannelAttn(channels, pooling_size))
            self.stn_list.append(nn.Linear(channels, 4))

    def stn(self, x, theta):
        grid = affine_grid(theta, x.size(), device=x.device.type, align_corners=True)
    ...
    def forward(self, features):
        ...
        for i in range(self.num_classes):
            ...
            stn_feature_size = [int(self.spatial), int(self.spatial//2)]
            theta_i = self.stn_list[i](F.avg_pool2d(stn_feature, stn_feature_size).view(bs,-1)).view(-1,4)
